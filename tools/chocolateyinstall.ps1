...
$url        = 'http://demo.sapphireims.com/SoftwareRepository/SapphireIMSAgent-5.0-Setup.exe' # download url, HTTPS preferred
$url64      = 'http://demo.sapphireims.com/SoftwareRepository/SapphireIMSAgent-5.0-Setup.exe' # 64bit URL here (HTTPS preferred) or remove - if installer contains both (very rare), use $url
...
$packageArgs = @{
  ...
  softwareName  = 'GoogleChromeTest*' #part or all of the Display Name as you see it in Programs and Features. It should be enough to be unique
  ...
  # MSI
  silentArgs    = "/mode "silent" /Password /ims /ServerIP localhost /ServerPort 80 /UserName sapphire" # ALLUSERS=1 DISABLEDESKTOPSHORTCUT=1 ADDDESKTOPICON=0 ADDSTARTMENU=0
